...
In Figure \ref{upp_tri}, we give a scheme of the region of interaction (highlighted in gray) between the basis functions in this case. These are the only regions in which \eqref{elem_noint_app} will be different than zero.
\begin{figure}[h]
\figinit{0.7pt}
% Axes
\figpt 0:(-90,-10)
...
These terms $B_i$, $i=1,2,3,4$, can be computed integrating by parts several times. Adding up the values obtained, we get
...
However, since we already have the expressions of $a_{i,j}$ for a general $s$, the second approach is actually straightforward and quicker. Indeed, we can easily compute
\begin{align*}
...
\subsubsection*{Computation of $Q_1$ and $Q_6$}
Since $\phi_i = 0$ on the domain of integration, one cane readily check that $Q_1=Q_6=0$. This is not surprising since, according to Figure \ref{upp_dia}, these regions of integration are outside of the region of interaction of the basis functions.

\subsubsection*{Computation of $Q_2$ and $Q_5$}
First of all notice that, due to symmetry (see Figure \ref{upp_dia}), we have $Q_2=Q_5$. Moreover, because of the interactions of the basis functions, we see that
\begin{align*}
	Q_2 &= 2\int_{x_{i+1}}^{+\infty}\int_{x_i}^{x_{i+1}} \frac{\phi_i(x)(\phi_{i+1}(x)-\phi_{i+1}(y))}{|x-y|^{1+2s}}\,dxdy. 
\end{align*}

Now, using Fubini's theorem for exchanging the order of the integrals, and employing the explicit expressions of the functions $\phi_i$, we get
\begin{align*}
...
The two integrals above can be computed explicitly by means of changes of variables analogous to \eqref{CV}. In this way, we obtain 
\begin{align*}
...
\begin{align}\label{Q3}
	...
	\\
	&= -2h^{1-2s}\int_0^1\int_{-1}^0 \frac{(1+x)(1-y)}{(y-x+1)^{1+2s}}\,dxdy = h^{1-2s}\frac{13-5\cdot 2^{3-2s}+3^{3-2s}+s(2^{4-2s}-14)+4s^2}{2s(1-2s)(1-s)(3-2s)},
\end{align}
...
\begin{align*}
	...
	\int_{x_i}^{x_{i+1}}\int_{x_{i-1}}^{x_i} \ldots\,dxdy +  \int_{x_i}^{x_{i+1}}\int_{x_i}^{x_{i+1}} \ldots\,dxdy = R_4^1 + R_4^2 + R_4^3 + R_4^4.
\end{align*}
...
\begin{align*}
	R_4 = 2(R_4^1+R_4^2) = \begin{cases}
					...
					\\[15pt]
					8-8\ln 2, & \displaystyle s=\frac{1}{2}.
				\end{cases}	
\end{align*}
